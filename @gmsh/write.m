function write(Gms,compound)
	...
	name=getgmsh(Gms,'name');
	%if abs(fid+1)<tol, error('failed to open file'); end
	runstring=['cat '];
	currpnts=lines=lineloops=surfaces=surfaceloops=volumes=physicalsurfaces=physicalline=0;
	surfs=[];
	...
	%	nsurfaces=surfaces+length(get(gms,'surfaces')(1,:)); end
		...
		write(gms,[currpnts lines lineloops surfaces surfaceloops volumes physicalsurfaces physicalline]);
